...
def add_to_conventional_data_store(room_id, query, mode):
    ...
    item = {'id': len(store), 'room_id': room_id, 'query': query, 'mode': mode, 'created_tx': time.time(), 'done': 0}
    store.append(item)
...
def add_request_to_panel(device_id, user_query, mode):
    ...
    # database_query = "INSERT INTO `requests` (room_id, request, created_tx, mode) VALUES ('%d', '%s', '%s',
    # '%d')" % (room_id, user_query, time.time(), mode) cursor = db.cursor() try: cursor.execute(database_query)
    # db.commit() # user_id = cursor.lastrowid success = True except e: db.rollback() success = False
